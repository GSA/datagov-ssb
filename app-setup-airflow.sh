#!/bin/bash
set -ex
APP_NAME=app-airflow
CSB_VERSION="v0.10.0"
DATAGOV_BROKERPAK_AIRFLOW_VERSION="v0.0.1"
...
# export SERVICE_NAME=ssb-airflow-k8s
cat > $APP_NAME/.profile << 'EOF'
# Locate additional binaries needed by the deployed brokerpaks
export PATH="$PATH:${PWD}/bin"

# Export credentials for the k8s cluster and namespace where the Airflow brokerpak
# should manage instances of SolrCloud. We get these from the binding directly.
export AIRFLOW_SERVER=$(echo $VCAP_SERVICES | jq -r '.[][]| select(.name=="ssb-airflow-k8s") | .credentials.server')
export AIRFLOW_CLUSTER_CA_CERTIFICATE=$(echo $VCAP_SERVICES | jq -r '.[][]| select(.name=="ssb-airflow-k8s") | .credentials.certificate_authority_data')
export AIRFLOW_TOKEN=$(echo $VCAP_SERVICES | jq -r '.[][]| select(.name=="ssb-airflow-k8s") | .credentials.token')
export AIRFLOW_NAMESPACE=$(echo $VCAP_SERVICES | jq -r '.[][]| select(.name=="ssb-airflow-k8s") | .credentials.namespace')
export AIRFLOW_DOMAIN_NAME=$(echo $VCAP_SERVICES | jq -r '.[][]| select(.name=="ssb-airflow-k8s") | .credentials.domain_name')
EOF
...
# Add the brokerpak(s)
(cd $APP_NAME && curl -f -LO https://github.com/GSA/datagov-brokerpak-airflow/releases/download/${DATAGOV_BROKERPAK_AIRFLOW_VERSION}/datagov-brokerpak-airflow-${DATAGOV_BROKERPAK_AIRFLOW_VERSION}.brokerpak)
...
cat > manifest-airflow.yml << MANIFEST
---
# Make a copy of vars-airflow-template.yml for each deployment target, editing the
# values to match your expectations. Then push with
#   cf push ssb-airflow -f manifest-airflow.yml --vars-file vars-airflow-ENV_NAME
applications:
- name: ssb-airflow
  path: app-airflow
  buildpacks:
  ...
  disk_quota: 2G
  routes:
  - route: ssb-airflow-((ORG))-((SPACE)).app.cloud.gov
  env:
    ...
    AWS_ZONE: ((AWS_ZONE))
MANIFEST
cat > vars-airflow-template.yml << VARS
