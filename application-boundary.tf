...
module "broker_sms" {
  ...
  name                  = "${var.name_prefix}-sms"
  path                  = "./app-sms"
  broker_space          = var.broker_space
  client_spaces         = var.client_spaces
  enable_ssh            = var.enable_ssh
  broker_db_plan_name   = var.broker_db_plan_name
  aws_access_key_id     = module.ssb-sms-broker-user.iam_access_key_id
  aws_secret_access_key = module.ssb-sms-broker-user.iam_access_key_secret
  aws_region            = var.aws_target_region
}
